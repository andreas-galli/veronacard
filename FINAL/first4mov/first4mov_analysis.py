...
# Generazione 'stats_first_4_movements', in cui si raggruppano i quadrupli spostamenti della giornata assegnando ad ognuno il numero di volte che si verifica
movements = pd.read_csv('first4mov/first_4_movements.csv')
...
for day in movements.iloc[:, 0].unique():
    ...
    for _, record in df_day.iterrows():
        if (day, record[2]) not in _4movements_counter:
            _4movements_counter[(day, record[2])] = 1
        else:
            _4movements_counter[(day, record[2])] += 1

result_df = pd.DataFrame([
    {'date': key[0], '4_movements': key[1], 'counter': value}
    for key, value in _4movements_counter.items()
])

result_df = result_df.sort_values(by=[result_df.columns[0], result_df.columns[2]], ascending=[True, False])

result_df.to_csv('first4mov/stats_first_4_movements.csv', index=False)

# Analisi file: stats_first_4_movements.csv
df = pd.read_csv('results/2018/flussi/3_clusters/2018_WEIGHT_clustering.csv')
df_stats = pd.read_csv('first4mov/stats_first_4_movements.csv')

_4movements_counter_cluster = {}

# Per le giornate appartenenti allo stesso cluster, si calcola il numero di volte che compare ogni quadruplo spostamento
    ...
    for date in df_stats.iloc[:, 0].unique():
        if (df_cluster == date).any():
            ...
            for _, record in df_stats_date.iterrows():
                if (cluster, record[1]) not in _4movements_counter_cluster:
                    _4movements_counter_cluster[(cluster, record[1])] = record[2]
                else:
                    _4movements_counter_cluster[(cluster, record[1])] += record[2]

result_df_cluster = pd.DataFrame([
    {'cluster': key[0], '4_movements': key[1], 'counter': value}
    for key, value in _4movements_counter_cluster.items()
])
...
# Separazione i risultati relativi alle occorrenze dei quadrupli spostamenti: un file per ogni cluster
result_df_cl0.to_csv('first4mov/CL0_first_4_movements.csv', index=False)
result_df_cl1.to_csv('first4mov/CL1_first_4_movements.csv', index=False)
result_df_cl2.to_csv('first4mov/CL2_first_4_movements.csv', index=False)
